...
typedef struct {
  ...
  int    seqLen ;
  char  *seq ;
  U8    *qual ;          // 0-based; length seqLen (NB may contain internal 0s)
  int    nameLen ;
  char  *name ;
  ...
  int    dustScore ;     // the dust score of this sequence, in range 0 to 100 (max dusty)
  char  *mLine ;         // contains reference substitutions for an alignment with maxScore
                         //     length seqLen ; positions matching seq are '.', deletions '-'
  int    nTax ;          // how many taxids this read has alignments to
  TaxID *taxid ;         // list of nTax taxids that have hits from this read
...
// cleans up memory, closes files; call on the base pointer (return value of oneReaderCreate)
...
// open a (threaded) outfile using oneFileOpenWriteFrom(infile) - call on the base pointer

void oneReaderWrite (OneReader *or) ;
// writes the current read object - call on a thread pointer

void oneReaderWriteTx (OneReader *or, Taxonomy *tx, bool *txUsed) ;
// writes the used part of the taxonomy at the end of the file - should be last before destroy
...
static char *schemaText =
  ...
  ".\n"
  "O U 4 3 INT 3 INT 3 INT 6 STRING   taxid, rank, parent, name\n"
  "O V 1 6 STRING               taxonomy rank\n"
  ".\n"
  ...
  "D D 1 3 INT                  dust score [0..100]\n"
  "D L 1 3 INT                  the lowest common ancestor of the taxids\n"
...
/*********************** finally routines in other files ***********************/
