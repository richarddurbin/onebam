 ...
 * Exported functions:
 * HISTORY:
 * Last edited: Aug 14 00:47 2025 (rd109)
...
static inline void transferLine (OneFile *ofOut, OneFile *ofIn)
{ memcpy (ofOut->field, ofIn->field, ofIn->info[ofIn->lineType]->nField*sizeof(OneField)) ;
...
bool merge1read (char *outfile, int nIn, char **infiles)
{
  ...
  for (i = 0 ; i < nIn ; ++i)
	...
	{ ++nSin ;
	  ...
	  while ((t = oneReadLine(in->of)) && (t != 'J')) ; // really should be next line...
	  if (!t) die ("no J line found in first read of %s", in->fileName) ;
	  if (oneInt(in->of,0) != 0)
	    die ("non-zero LCP %d on first read of %s", oneInt(in->of,0), in->fileName) ;
	  in->lcp = 0 ;
	  if (oneLen(in->of) > 1024)
	    die ("first read name length %d > 1024 file %s", oneLen(in->of), in->fileName) ;
  ...
  while ((nName = mergeNext (readMerge, &nameList)))
    { Input *in = &inputs[nameList[0]] ; // this is an example to copy the shared information from
      ...
      // next copy all lines up to M, which should be the same across all inputs
	...
	{ Input *in = &inputs[nameList[i]] ;
	  if (!in->of->lineType) in->of = 0 ; // terminator for the txMerge
	  else if (in->of->lineType != 'S')
	  ...
	  in->dna = oneDNA2bit(in->of) ;
	  while ((t = oneReadLine(in->of)) && (t != 'J')) ; // really should be next line...
	  if (!t) die ("no J line found in %s", in->fileName) ;
